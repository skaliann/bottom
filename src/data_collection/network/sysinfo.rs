...
pub fn get_network_data(
    networks: &Networks, prev_net_access_time: Instant, 
    prev_net_rx: &mut u64, prev_net_tx: &mut u64, 
    prev_net_rx_pkts: &mut u64, prev_net_tx_pkts: &mut u64,
    ...
    for (name, network) in networks {
        ...
        if to_keep {
            ...
            total_rx_pkts += network.total_packets_received();
            total_tx_pkts += network.total_packets_transmitted();
        }
    }

    let elapsed_time = curr_time.duration_since(prev_net_access_time).as_secs_f64();

    let (rx, tx, rx_pkts, tx_pkts) = if elapsed_time == 0.0 {
        (0, 0, 0, 0)
    } else {
        (
            ...
            ((total_rx_pkts.saturating_sub(*prev_net_rx_pkts)) as f64 / elapsed_time) as u64,
            ((total_tx_pkts.saturating_sub(*prev_net_tx_pkts)) as f64 / elapsed_time) as u64,
        )
    };

    *prev_net_rx = total_rx;
    *prev_net_tx = total_tx;
    *prev_net_rx_pkts = total_rx_pkts;
    *prev_net_tx_pkts = total_tx_pkts;
